...
/**
 * A custom implementation of a {@link org.elasticsearch.xpack.security.authc.support.CachingRealm} that shows
 * what is necessary to integrate with the X-Pack cache eviction APIs. A realm may need to cache data for
 * performance reasons and if the cached data is changed in an external system, the cache may need to be
 * expired. By implementing the {@link CachingRealm} interface, the X-Pack cache eviction
 * API can be used to clear a user or the entire cache.
 *
 * This class merely extends the existing {@link org.elasticsearch.xpack.security.authc.support.CachingRealm}
 * and implements a cache on top of the realm with a regular {@link ConcurrentMap}. An actual cache
 * implementation will be required for features such as LRU and time based evictions with a size limit.
 */
public class KerberosCachingRealm extends KerberosRealm implements CachingRealm {

    public static final String TYPE = "caching-cc-kerberos";

    private final ConcurrentMap<String, UserHolder> cache = new ConcurrentHashMap<>();

    public KerberosCachingRealm(RealmConfig config) {
        super(TYPE, config);
    }
